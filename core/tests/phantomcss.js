...
function turnOffAnimations() {
	...
	casper.evaluate( function turnOffAnimations() {
		...
		function disableAnimations() {
			...
			css.type = "text/css";
			css.innerHTML = "* { -webkit-transition: none !important; transition: none !important; -webkit-animation: none !important; animation: none !important; ; visibility: visible;  -webkit-transform: initial!important; opacity: 1!important;transform: initial!important;} .wpb_revslider_element{display: none!important;} .header--v1 .header__row {background: red;}";
			document.body.appendChild( css );
			console.log('must insert css')
		}
...
function setVisibilityToHidden( s1, s2 ) {
	...
	jQuery('[data-sr-id]').removeAttr('style')

	if ( jQuery ) {
		console.log(s1)
		console.log(s2)
		if ( s1 ) {
			jQuery( s1 ).css( { 'visibility': 'hidden!important', 'display': 'none!important' });
            jQuery( s1 ).remove();
		}
		if ( s2 ) {
			jQuery( s2 ).css( { 'visibility': 'hidden!important', 'display': 'none!important' });
            jQuery( s2 ).remove();
	...
	s1 = s1 || '';
	s2 = s2 || '';
    console.log('+++++++++setVisibilityToHidden')
